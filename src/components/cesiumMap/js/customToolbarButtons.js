...
export function createCustomToolbarButtons() {
   ...
   * @returns 创建的按钮元素
   */
  const addCustomToolbarButton = (viewer, options) => {
   ...
   * @returns 创建的按钮元素
   */
  const addCustomIconToolbarButton = (viewer, options) => {
    const toolbar = viewer.container.querySelector('.cesium-viewer-toolbar')
   ...
   * @returns 创建的按钮元素数组
   */
  const addCustomToolbarButtons = (viewer, buttons) => {
   ...
   */
  const removeCustomToolbarButtons = (buttons) => {
