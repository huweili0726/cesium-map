...
export function geometryConfig() {
  ...
  const rectangularPyramidWave1 = (options) => {
    ...
    let instanceGeo = new Cesium.GeometryInstance({
      geometry: new Cesium.FrustumGeometry({
        frustum: frustum,
        origin: Cesium.Cartesian3.ZERO,        // 改为 ZERO
        orientation: Cesium.Quaternion.IDENTITY, // 改为 IDENTITY
    ...
    const rectangularPrimitive = new Cesium.Primitive({
    ...
    const rectangularPrimitive1 = new Cesium.Primitive({
    ...
    rectangularPrimitive.modelMatrix = modelMatrix;
    rectangularPrimitive1.modelMatrix = modelMatrix;

    // 存储参数和矩阵（供后续快速更新使用）
    rectangularPrimitive._originalOptions = { ...options };
    rectangularPrimitive._linePrimitive = rectangularPrimitive1;
    rectangularPrimitive._rotationMatrix = rotationMatrix; // 缓存旋转矩阵（可选）
    rectangularPrimitive._origin = origin; // 缓存原点（可选）
  ...
  const updateRectangularPyramidWavePose1 = (options) => {
    ...
    try {
      ...
      // 2. 显式销毁WebGL资源（关键！）
      if (!existingPrimitive.isDestroyed()) {
        existingPrimitive.destroy();
      }
      if (!existingPrimitiveLine.isDestroyed()) {
        existingPrimitiveLine.destroy();
      }
      ...
      // 4. 获取合并后的参数
      const originalOptions = existingPrimitive._originalOptions || {};
  ...
  const updateRectangularPyramidPoseFast = (options) => {
    ...
    try {
      ...
      const positions = options?.positions || primitive._originalOptions?.positions || [0, 0, 0];
      ...
      // 更新存储的参数（供后续逻辑使用）
      primitive._originalOptions.heading = heading;
      primitive._originalOptions.pitch = pitch;
